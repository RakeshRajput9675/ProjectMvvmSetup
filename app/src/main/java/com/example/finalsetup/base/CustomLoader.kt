package com.example.finalsetup.base
...
class CustomLoader(context: Context, theme: Int) : Dialog(context) {
    companion object {
        ...
        fun show(context: Context?, cancelable: Boolean): CustomLoader? {
            ...
            // Prevent leak if activity is invalid
            if (activity == null || activity.isFinishing || activity.isDestroyed) return null

            return try {
                val dialog = CustomLoader(activity, android.R.style.Theme_Black)
